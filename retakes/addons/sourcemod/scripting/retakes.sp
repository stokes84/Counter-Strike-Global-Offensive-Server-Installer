...
public OnPluginStart()
{
	...
	//Admin Commands
	RegAdminCmd("sm_add", Command_AddSpawn, ADMFLAG_GENERIC);
	RegAdminCmd("sm_start", Command_Start, ADMFLAG_GENERIC);
	RegAdminCmd("sm_del", Command_DeleteSpawn, ADMFLAG_GENERIC);
	RegAdminCmd("sm_delete", Command_DeleteSpawn, ADMFLAG_GENERIC);
	RegAdminCmd("sm_edit", Command_Edit, ADMFLAG_GENERIC);
	RegAdminCmd("sm_spawns", Command_Spawns, ADMFLAG_GENERIC);
	...
	AddCommandListener(Hook_ChangeTeam, "teammenu");
	//Offsets
...
public Event_OnRoundPreStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{	
		...
		for (new i = 1; i <= MaxClients; i++)
		{
			...
			{
				CS_SwitchTeam(i, gi_NextRoundTeam[i]);
...
public Event_OnRoundPostStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(!gb_EditMode && !gb_WaitForPlayers && !gb_WarmUp)
	{
		...
		new bool:pistols = CS_GetTeamScore(3) + CS_GetTeamScore(2) < gi_PistolRoundsCount;
		for (new i = 1; i <= MaxClients; i++)
		{
	...
	if(!gb_EditMode)
	{
		...
		else
		{
			if(GetEventInt(event, "winner") == 3)
			{
				...
				new IndexList[MaxClients];
				new count = 0;
				new terrorCount = RoundToFloor(GetClientCountFix(true) / 2.0);
				if(terrorCount > 5)
					terrorCount = 5;
				for (new i = 1; i <= MaxClients; i++)
				{
...
ScrambleTeams()
{
	...
	new terrorCount = RoundToFloor(GetClientCountFix() / 2.0);
	if(terrorCount > 5)
		terrorCount = 5;
	new randomPlayer = -1;
	for (new i = 0; i < terrorCount; i++)
	...
	for (new i = 1; i < MaxClients; i++)
	{
		if(IsClientInGame(i) && gi_NextRoundTeam[i] != 2)
		{
			gi_NextRoundTeam[i] = 3;
		}
	}
...
public Action:Timer_PrintHintSite(Handle:timer, any:site)
{
	if(GetEngineTime() - gf_StartRoundTime >= 3)
		return Plugin_Stop;
	PrintHintTextToAll("\n<font size='22'>Retake on Site:</font><font size='22' color='#E22C56'>%s</font>", site == SITEA ? "A":"B");
	return Plugin_Continue;
}

SetConfig(bool:warmup)
{
	//AutoExecConfig(false, "retakes", "");
	ServerCommand("exec retakes.cfg");
	if(!warmup)
	{
		//AutoExecConfig(false, "retakes_live", "");
		ServerCommand("exec retakes_live.cfg");
	}
	else
	{
		//AutoExecConfig(false, "retakes_warmup", "");
